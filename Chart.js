  ...
  var Line = function(data,config,ctx){
    ...
    function drawLines(animPc){
      for (var i=0; i<data.datasets.length; i++){
        ...
        if(config.pointDot || data.datasets[i].pointDot){
          ...
          ctx.lineWidth = config.pointDotStrokeWidth;
          for (var k=0; k<data.datasets[i].data.length; k++){
            if (data.datasets[i].data[k] instanceof Array) {
              for (var l in data.datasets[i].data[k]) {
                placePoint(data.datasets[i].data[k][l]);
              }
            } else {
              placePoint(data.datasets[i].data[k]);
            }
          }
        } 
      }
      ...
      function placePoint (point) {
        if (typeof point == 'number') {
          ctx.beginPath();
          ctx.arc(yAxisPosX + (valueHop *k),xAxisPosY - animPc*(calculateOffset(getPointValue(point),calculatedScale,scaleHop)),config.pointDotRadius,0,Math.PI*2,true);
          ctx.fill();
          ctx.stroke();
        } else if (typeof point.value == 'number' && (typeof point.pointImage != 'undefined' || config.pointImage)) {
          var image = new Image();
          image.src = point.pointImage ? point.pointImage : config.pointImage;
          ctx.drawImage(
            image,
            (yAxisPosX + (valueHop *k)) - (config.pointImageWidth/2),
            (xAxisPosY - animPc*(calculateOffset(getPointValue(point),calculatedScale,scaleHop))) - (config.pointImageHeight/2),
    ...
    function drawScale(){
      ...
      for (var j=0; j<calculatedScale.steps; j++){
        ...
        if (config.scaleShowLabels){
          if (typeof data.scaleLabelTransform == 'function') {
            ctx.fillText(data.scaleLabelTransform(calculatedScale.labels[j]),yAxisPosX-8,xAxisPosY - ((j+1) * scaleHop));
          } else {
